...
private:
    ...
    void open_table() {
        ...
        if (!cfg->is_recovery) {
            for (const auto &entry :
                ...
                // std::cout << "__AA__" << entry.path().string() << std::endl;
    ...
    void finish() {
        ...
        for (const auto &entry :
             std::filesystem::directory_iterator(cfg->working_dir)){
            std::cout << "__BB__" << entry.path().string() << std::endl;
            // std::filesystem::remove_all(entry.path());}
            }
    }
};
